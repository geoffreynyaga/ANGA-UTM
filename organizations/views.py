...
class AllCompanyFlightLogs(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["my_affiliate_companies"] = Organization.objects.filter(
            users=self.request.user
        )
        context["organization_and_users"] = all_users
        context[
            "all_company_logs"
        ] = all_company_logs  # TODO: Company Logs: list only 5 and have a link to a table of all logs
...
class PostholderCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        form = form
        # reserveairspace.created_by = User.objects.get(email=self.request.user)  # use your own profile here
        # reserveairspace.save()
        from utm_messages.models import UserToUserMessages

        UserToUserMessages.objects.create(
            title="New Position",

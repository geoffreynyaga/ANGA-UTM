...
from .models import Manufacturer, Payload, Rpas, RpasModel


@login_required()
def home(request):
    x = (
        UserToUserMessages.objects.filter(receiver=request.user)
    ...
    unread_notifications = (
        Notifications.objects.filter(receiver=request.user)
        .filter(is_read=False)
        .order_by("-id")[:5]
    )
    ...
    all_rpas_tasks_count = len(unfinished_rpas_payload_tasks) + len(
        unfinished_rpas_model_tasks
    )
    ...
    all_flightlog_tasks_count = len(unfinished_pre_flight_logs) + len(
        unfinished_post_flight_logs
    )
...
class RpasCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        rpas.user = self.request.user

        # Fetch organization from userprofile
        if hasattr(self.request.user, "userprofile") and self.request.user.userprofile.organization:
            rpas.organization = self.request.user.userprofile.organization
        else:
            # If no organization, we should ideally handle this,
            # but the template will also check this.
            pass
...
def rpas_manufacturer_detail(request, rpas_pk, manufacturer_pk):
    ...
    return render(
        request, "rpas/manufacturer_details.html", {"manufacturer": manufacturer}
    )

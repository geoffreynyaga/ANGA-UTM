...
class Manufacturer(models.Model):
    ...
    logo = models.ImageField(
        upload_to="images/logo/manufacturer", blank=True, null=True
    )
...
class RpasModel(models.Model):
    ...
    rpas_model_type = models.ForeignKey(
        RpasModelType, on_delete=models.CASCADE, blank=True, null=True
    )
...
class Payload(models.Model):
    ...
    payload_nickname = models.CharField(
        max_length=20, blank=True, null=True, help_text="if any....."
    )
...
class Rpas(models.Model):
    ...
    rpas_model = models.ForeignKey(
        RpasModel, on_delete=models.CASCADE, blank=True, null=True
    )
    payload = models.ForeignKey(
        Payload, on_delete=models.CASCADE, blank=True, null=True
    )
    ...
    def get_payload_completion(self):
        ...
        for field in fields:
            if (
                field == "" or field == None
            ):  # FIXME: PYCHARM suggests that its: field is None
                fields.remove(field)
        final_count = int(len(fields))
        progress = (final_count / initial_count) * 100
        return round(progress, 1)

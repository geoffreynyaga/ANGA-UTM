...
class ReserveAirspaceCreateView(CreateView):
    ...
    def get_context_data(self,*args,**kwargs):
        context = super(ReserveAirspaceCreateView,
                        self).get_context_data(*args, **kwargs)

        my_pending_airspaces = ReserveAirspace.objects.filter(
            created_by=self.request.user).filter(status=0)

        context['my_pending_approval_airspaces'] = my_pending_airspaces.order_by('-id')[:10]
...
# TO DO: KCAA LOGIN REQUIRED MIXIN
# --FIXED: templates has_group=='KCAA'
class AppliedReserveAirspaceListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        approved_airspaces = ReserveAirspace.objects.filter(status=2).order_by(
            '-id')

        rejected_airspaces = ReserveAirspace.objects.filter(status=1).order_by(
            '-id')

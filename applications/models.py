...
class Project(models.Model):
    ...
    target_bags = models.IntegerField(default=0)
    target_spray_acres = models.IntegerField(default=0)
    estimated_ops_budget = models.FloatField(default=0.0)
...
class ReserveAirspace(gis_models.Model):
    ...
    def get_airframe_type(self):
        return ", ".join(
            list(set([str(r.rpas_model.rpas_model_type) for r in self.rpas.all()]))
        )
